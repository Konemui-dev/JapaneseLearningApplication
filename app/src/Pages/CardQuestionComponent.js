...
function CardQuestionComponent(value) {
  ...
  const CheckQuestion = (e) => {
    if (
      Object.values(value.props.senses[0].english_definitions).indexOf(e) > -1
    ) {
      setAnswerR(true);
      setAnswerW(false);
      setDisable(true);
  ...
  const statusR = <Typography>correct!</Typography>;

  const statusW = <Typography gutterBottom>incorrect try again</Typography>;
  ...
  return (
    ...
    >
      <CardContent sx={{ color: "#1BA098", textAlign: "center" }}>
        ...
        {/**demo code disable permanently if not using */}{" "}
        {value.dev && (
          <div>
